...
export function AppShell({
  ...
  const nav = profile?.mode === "expense" ? expenseNav : jobNav;
  ...
  return (
    <div className="min-h-screen pb-28">
        ...
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 px-4 py-3">
          <div className="min-w-0">
            <p className="truncate text-xs font-medium uppercase tracking-wide text-muted-foreground">
              {profile?.business_name || "Your business"} · {profile?.mode === "expense" ? "Expense mode" : "Job mode"}
            </p>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 backdrop-blur">
        <div className="mx-auto grid max-w-5xl grid-cols-5 items-end gap-1 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2">
