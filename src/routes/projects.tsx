...
export const Route = createFileRoute("/projects")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Track budget, actual cost and gross margin for every job you run." },
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              <div className="space-y-1.5">
                ...
                <Input id="np" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              </div>
              <div className="space-y-1.5">
                <Label htmlFor="nc">Customer</Label>
                <Input id="nc" value={form.customer} onChange={(e) => setForm({ ...form, customer: e.target.value })} />
              </div>
    ...
    >
      {isLoading ? (
        ...
        <div className="panel p-8 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Add a job to start tracking costs against a budget.</p>
        </div>
      ) : (
                ...
                <Link to="/projects/$id" params={{ id: p.id }} className="panel block p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs text-muted-foreground">{p.customer || "No customer"}</p>
                    </div>
